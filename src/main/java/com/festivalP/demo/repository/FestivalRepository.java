...
//    제목 검색할 때 필요
    ...
    //지역별로 데이터찾기
    public List<Posts> findByboardLocAddr(Long boardLocAddr) {
        return em.createQuery("select p from Posts p where p.boardLocAddr LIKE :boardLocAddr", Posts.class).setParameter("boardLocAddr",boardLocAddr).getResultList();
    }
